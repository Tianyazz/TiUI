...
const transformModel = (dir, node, context) => {
    ...
    if (maybeRef) {
        if (bindingType === "setup-ref" /* BindingTypes.SETUP_REF */) {
            ...
            const altAssignment = bindingType === "setup-let" /* BindingTypes.SETUP_LET */
                ? `${rawExp} = ${eventValue}`
                : ``;
            assignmentExp = (0, compiler_core_1.createCompoundExpression)([
                `${eventArg} => {if(${context.helperString(compiler_core_1.IS_REF)}(${rawExp})) { (`,
                (0, compiler_core_1.createSimpleExpression)(rawExp, false, exp.loc),
                `).value = ${eventValue} }${altAssignment ? ` else { ${altAssignment} }` : ``}}`,
            ]);
        }
    }
    else {
    ...
    if (dir.modifiers.length && node.tagType === 1 /* ElementTypes.COMPONENT */) {
        ...
        const modifiersKey = arg
            ...
            : `modelModifiers`;
        props.push((0, compiler_core_1.createObjectProperty)(modifiersKey, (0, compiler_core_1.createSimpleExpression)(`{ ${modifiers} }`, false, dir.loc, 2 /* ConstantTypes.CAN_HOIST */)));
    }
