...
function processDefineModel(ctx, node, declId) {
    ...
    ctx.bindingMetadata[modelName] = "props" /* BindingTypes.PROPS */;
    const runtimeTypes = type && (0, resolveType_1.inferRuntimeType)(ctx, type);
...
function genModelProps(ctx) {
    ...
    for (const [name, { type, options }] of Object.entries(ctx.modelDecls)) {
        let skipCheck = false;
        let runtimeTypes = type && (0, resolveType_1.inferRuntimeType)(ctx, type);
        if (runtimeTypes) {
