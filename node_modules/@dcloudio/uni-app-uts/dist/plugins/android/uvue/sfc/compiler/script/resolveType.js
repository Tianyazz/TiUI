...
function inferRuntimeType(ctx, node, scope = node._ownerScope || ctxToScope(ctx)) {
    try {
        switch (node.type) {
                    ...
                    switch (node.typeName.name) {
                        ...
                        case 'NonNullable':
                            if (node.typeParameters && node.typeParameters.params[0]) {
                                return inferRuntimeType(ctx, node.typeParameters.params[0], scope).filter((t) => t !== 'null');
                            }
                            break;
                        case 'Extract':
            ...
            case 'TSIntersectionType': {
                return flattenTypes(ctx, node.types, scope).filter((t) => t !== utils_1.UNKNOWN_TYPE);
            }
            case 'TSEnumDeclaration':
...
function flattenTypes(ctx, types, scope) {
    ...
    return [
        ...new Set([].concat(...types.map((t) => inferRuntimeType(ctx, t, scope)))),
    ];
}
